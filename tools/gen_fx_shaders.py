...
Marker convention (shared with tools/gen_fx_art.py):
    ...
    FX ids: 1 prism core, 2 prism slash, 3 (unused), 4 nova ring,
            ...
            16 capsule impostor (axis rebuilt from dFdx/dFdy -> glowing rod; works
               in hand; sub 0 held/orange (params ignored), 1 fire, 2 void,
               3 orange display; param.y fade for subs 1..3),
    ...
    Sphere FX (14/15/17/18/19) use the domain-expansion canvas technique: the model is a
    full cube, the plugin spawns it with NEGATIVE scale (mirrored -> faces visible from
    inside and the far faces from outside), and the fsh rebuilds each face's frame from
    screen derivatives (two candidate vertex-label orders), derives the cube centre and
    ray-casts the sphere. No vsh billboard, no scale assumptions, no gl_VertexID trust.
...
float og_ndc_z(vec3 v) {
    vec4 c = ProjMat * vec4(v, 1.0);
    return c.z / c.w;
}

// Write the depth of view-space point p instead of the billboard quad's: expressed as an
// offset from gl_FragCoord.z so the depth-range convention cancels out.
void og_depth(vec3 p) {
    gl_FragDepth = clamp(gl_FragCoord.z + (og_ndc_z(p) - og_ndc_z(ogPosV)) * 0.5, 0.0, 1.0);
}
...
vec4 og_fx(vec4 tex) {
    ...
    if (ogFx == 16) {
        ...
        // sub 0 = held item (UV2 is real light, not our params); 1..3 = displays with a fade param
        float fadeC = ogSubI == 0 ? 0.0 : ogParam.y;
        ...
        float a = clamp((core + halo * 0.9) * hum, 0.0, 1.0) * (0.35 + 0.65 * facing) * (1.0 - fadeC);
        if (a < 0.02) {
            return vec4(0.0);
        }
        return vec4(c, a);
    ...
    if (ogFx == 19) {
        ...
        og_depth(rd * (b + r)); // halo sits behind the sphere so the disc's far half shows
        vec3 closest = rd * b;
